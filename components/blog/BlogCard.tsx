...
export function BlogCard({ post, featured = false, compact = false, priority = false, className }: BlogCardProps) {
  ...
  return (
    <article
      className={cn(
        "group relative h-full overflow-hidden rounded-lg border border-[var(--blog-border)] bg-[var(--blog-card)] shadow-[var(--blog-shadow)] transition-all duration-300 before:pointer-events-none before:absolute before:inset-0 before:z-10 before:bg-[radial-gradient(circle_at_18%_0%,rgba(109,204,18,0.12),transparent_34%)] before:opacity-0 before:transition-opacity before:duration-300 hover:-translate-y-1 hover:border-violet/35 hover:shadow-[var(--blog-shadow-strong)] hover:before:opacity-100",
        className,
      )}
    >
      <Link href={`/blog/${post.slug}`} className="block h-full" aria-label={`Read ${post.title}`}>
        <div
          className={cn(
            "relative overflow-hidden bg-[var(--blog-panel-soft)]",
        ...
        >
          ...
          <span className="absolute left-4 top-4 rounded-full border border-violet/20 bg-[var(--blog-panel)]/88 px-3 py-1 font-mono text-[10px] font-medium uppercase tracking-widest text-violet shadow-sm backdrop-blur-xl">
            {category}
          </span>
        </div>

        <div className={cn("relative z-20 flex flex-col", featured ? "p-6 md:p-7" : compact ? "p-4 md:p-5" : "p-5")}>
          <div className="flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-[var(--blog-faint)]">
          ...
          <h3
            className={cn(
              "mt-4 line-clamp-2 font-display font-semibold leading-tight text-[var(--blog-text)] transition-colors group-hover:text-violet",
              featured ? "text-2xl md:text-4xl" : compact ? "text-lg md:text-xl" : "text-xl md:text-2xl",
            )}
          >
            {post.title}
          </h3>
          <p className={cn("mt-3 text-sm leading-relaxed text-[var(--blog-muted)]", compact ? "line-clamp-2" : "line-clamp-3", featured && "md:text-base")}>
            {post.excerpt}
          </p>
